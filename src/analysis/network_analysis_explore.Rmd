...
df_graphs <- df_trials_weights %>% 
  ...
  mutate(transitivity_weighted = map_dbl(graph, ~mean(transitivity(., "weighted")))) %>% 
...
## PAQ Neg 
df_graphs_survey %>% 
  ggplot(aes(x = paq_neg, y = transitivity_weighted, color = trial_valence)) +   geom_point(alpha = .3) + 
  geom_smooth(method="lm")
df_graphs_survey %>% 
  filter(trial_valence == "negative") %>% 
  mutate_if(is.numeric, scale) %>% 
...
df_graphs_survey %>% 
  filter(trial_valence == "positive") %>% 
  mutate_if(is.numeric, scale) %>%   lm(transitivity_weighted ~ paq_neg, .) %>% 
  summary
